        ...
        <section className="panel progress-panel"><div className="section-title"><h2>回付ルート・履歴</h2>{canEdit && <button className="secondary-button" onClick={() => setEditingRoute(true)}>未処理ルートを編集</button>}</div>
          ...
          <div className="timeline">
            ...
            {item.members.map((member, index) => {
              const completed = member.status === "approved" || member.status === "stamped";
              const memberHistory = item.history.filter((entry) => entry.actionUserId === member.approverId && !["回覧開始", "再回覧", "回付ルート変更"].includes(entry.action));
              const returnHistory = memberHistory.filter((entry) => entry.action.includes("NG"));
              const processedAt = member.completedAt ?? memberHistory.filter((entry) => !entry.action.includes("NG")).at(-1)?.createdAt;
              return <div key={member.id} className={`timeline-item ${member.status}`}><div className="timeline-marker"><span>{completed ? "✓" : member.status === "current" ? "●" : index + 1}</span>{index < item.members.length - 1 && <i />}</div><RouteTypeIcon requiresStamp={member.requiresStamp} /><div className="timeline-person"><div className="timeline-person-heading"><div><strong>{member.name}</strong><small>{member.department} ・ {member.email}</small></div><div className="timeline-state"><strong>{member.status === "stamped" ? "捺印・確認済み" : member.status === "approved" ? "確認済み" : member.status === "current" ? "処理中" : "未処理"}</strong>{processedAt && <small>{formatDate(processedAt)}</small>}</div></div>{returnHistory.length > 0 && <div className="member-history">{returnHistory.map((entry) => <div className="is-return" key={entry.id}><span>差し戻し</span><time>{formatDate(entry.createdAt)}</time>{entry.returnToUserName && <p>戻り先：{entry.returnToUserName}</p>}{entry.comment && <blockquote>{entry.comment}</blockquote>}</div>)}</div>}</div></div>;
            })}
          </div>
        </section>
      </div>
