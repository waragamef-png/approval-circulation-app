...
import { fileNameFromSharedFolderPath, normalizeSharedFolderPath, sharedFolderOpenUrl } from "./services/sharedFolder";
import type {
  AppData,
  Approver,
  CaseDocument,
...
type LinkProvider = Extract<ProviderType, "sharepoint" | "onedrive">;

function documentLinkUrl(value: string, provider?: LinkProvider) {
  try {
    const url = new URL(value.trim());
    const hostname = url.hostname.toLowerCase();
    const isSharePoint = hostname === "sharepoint.com" || hostname.endsWith(".sharepoint.com");
    const isOneDrive = isSharePoint || hostname === "1drv.ms" || hostname === "onedrive.live.com";
    if (url.protocol !== "https:" || (provider === "sharepoint" ? !isSharePoint : !isOneDrive)) return undefined;
    return url.toString();
  } catch {
    return undefined;
  }
...
function fileNameFromDocumentUrl(value: string, provider: LinkProvider) {
  const normalized = documentLinkUrl(value, provider);
...
function DocumentOpenLink({ document, provider, notify }: { document: CaseDocument; provider: ProviderType; notify: (message: string) => void }) {
  const url = provider === "shared-folder" ? sharedFolderOpenUrl(document.fileUrl) : documentLinkUrl(document.fileUrl);
  if (!url) {
...
function NewCirculationPage({ data, currentUser, setTemplates, onStart, notify }: { data: AppData; currentUser: Approver; setTemplates: (items: RouteTemplate[]) => void; onStart: (item: CirculationCase) => void; notify: (message: string) => void }) {
  ...
  return <div className="page">
    <PageHeading title="新しい回覧" help={<ol><li>SharePoint／OneDriveは文書リンク、共有フォルダは完全なファイルパスを追加します。</li><li>文書ごとの捺印要否を設定します。</li><li>開始者はルートの先頭へ自動で入り、同じ人も複数回追加できます。</li><li>承認者を追加し、ドラッグまたは「上へ／下へ」で並べ替えます。</li></ol>} />
    <div className="builder-main">
        <section className="panel form-section"><label className="circulation-name-field"><span className="field-label">回覧名（任意）</span><input value={circulationName} onChange={(event) => setCirculationName(event.target.value)} placeholder="未入力の場合は文書名を表示" maxLength={100} /><small>{circulationName.length} / 100</small></label><div className="number-title"><b>1</b><h2>対象文書</h2></div><div className="document-source"><label><span className="field-label">保存場所</span><select value={provider} onChange={(event) => { setProvider(event.target.value as ProviderType); setSelectedDocuments([]); }}><option value="sharepoint">SharePoint</option><option value="onedrive">OneDrive</option><option value="shared-folder">共有フォルダ</option></select></label><button className="secondary-button" onClick={() => setFilePicker(true)}>{provider === "shared-folder" ? "ファイルパスを追加" : `${providerLabels[provider]}リンクを追加`}</button></div>
    ...
    {filePicker && provider !== "shared-folder" && <DocumentLinkModal provider={provider} onClose={() => setFilePicker(false)} onAdd={(file) => { const label = providerLabels[provider]; if (selectedDocuments.some((document) => document.fileUrl === file.fileUrl)) { notify(`同じ${label}リンクは追加済みです`); return; } setSelectedDocuments([...selectedDocuments, { id: uid("document"), fileId: file.id, name: file.name, type: file.type, location: file.location, fileUrl: file.fileUrl, requiresStamp: false }]); setFilePicker(false); notify(`${label}文書を追加しました`); }} />}
    {filePicker && provider === "shared-folder" && <SharedFolderFileModal onClose={() => setFilePicker(false)} onAdd={(file) => { if (selectedDocuments.some((document) => document.fileUrl.toLowerCase() === file.fileUrl.toLowerCase())) { notify("同じファイルパスは追加済みです"); return; } setSelectedDocuments([...selectedDocuments, { id: uid("document"), fileId: file.id, name: file.name, type: file.type, location: file.location, fileUrl: file.fileUrl, requiresStamp: false }]); setFilePicker(false); notify("共有フォルダ文書を追加しました"); }} />}
...
function DocumentLinkModal({ provider, onClose, onAdd }: { provider: LinkProvider; onClose: () => void; onAdd: (file: MockFile) => void }) {
  const [link, setLink] = useState("");
  const [name, setName] = useState("");
  const [nameEdited, setNameEdited] = useState(false);
  const [error, setError] = useState("");
  const label = providerLabels[provider];
  const normalizedLink = documentLinkUrl(link, provider);
  const canAdd = Boolean(link.trim() && name.trim());
  const submit = (event: FormEvent) => {
    event.preventDefault();
    if (!normalizedLink) { setError(`${label}でコピーしたhttpsリンクを貼り付けてください`); return; }
    if (!name.trim()) { setError("ファイル名を入力してください"); return; }
    onAdd({ id: uid(`${provider}-link`), provider, name: name.trim(), type: fileTypeFromName(name), location: label, updatedAt: "", updatedBy: "", fileUrl: normalizedLink });
  };
  return <Modal title={`${label}文書を追加`} onClose={onClose}><form onSubmit={submit}><div className="modal-body form-grid"><label><span>{label}リンク <b>必須</b></span><input type="url" value={link} onChange={(event) => { const value = event.target.value; setLink(value); setError(""); if (!nameEdited) setName(fileNameFromDocumentUrl(value, provider)); }} placeholder="コピーしたリンクを貼り付け" autoFocus /></label><label><span>ファイル名 <b>必須</b></span><input value={name} onChange={(event) => { setName(event.target.value); setNameEdited(true); setError(""); }} placeholder="例：製品仕様書.pdf" /></label>{error && <div className="suggestion-error">{error}</div>}</div><div className="modal-footer"><button type="button" className="quiet-button" onClick={onClose}>キャンセル</button><button type="submit" className="primary-button" disabled={!canAdd}>追加する</button></div></form></Modal>;
}

function SharedFolderFileModal({ onClose, onAdd }: { onClose: () => void; onAdd: (file: MockFile) => void }) {
  const [filePath, setFilePath] = useState("");
  const [name, setName] = useState("");
  const [nameEdited, setNameEdited] = useState(false);
  const [error, setError] = useState("");
  const normalizedPath = normalizeSharedFolderPath(filePath);
  const canAdd = Boolean(filePath.trim() && name.trim());
  const submit = (event: FormEvent) => {
    event.preventDefault();
    if (!normalizedPath) { setError("共有フォルダ内の完全なファイルパスを入力してください"); return; }
    if (!name.trim()) { setError("ファイル名を入力してください"); return; }
    onAdd({ id: uid("shared-folder-file"), provider: "shared-folder", name: name.trim(), type: fileTypeFromName(name), location: "共有フォルダ", updatedAt: "", updatedBy: "", fileUrl: normalizedPath });
  };
  return <Modal title="共有フォルダ文書を追加" onClose={onClose}><form onSubmit={submit}><div className="modal-body form-grid"><label><span>ファイルパス <b>必須</b></span><input value={filePath} onChange={(event) => { const value = event.target.value; setFilePath(value); setError(""); if (!nameEdited) setName(fileNameFromSharedFolderPath(value)); }} placeholder="例：\\\\fileserver\\共有\\製品仕様書.pdf" autoFocus /></label><label><span>ファイル名 <b>必須</b></span><input value={name} onChange={(event) => { setName(event.target.value); setNameEdited(true); setError(""); }} placeholder="例：製品仕様書.pdf" /></label>{error && <div className="suggestion-error">{error}</div>}</div><div className="modal-footer"><button type="button" className="quiet-button" onClick={onClose}>キャンセル</button><button type="submit" className="primary-button" disabled={!canAdd}>追加する</button></div></form></Modal>;
}
