...
SharePoint、OneDrive、社内共有フォルダに保存された業務ファイルを、案件ごとの専用URLで順番に承認回覧するWebアプリです。

デモ版：<https://waragamef-png.github.io/approval-circulation-app/>

> この公開版は画面・操作確認専用です。初期表示される氏名、メールアドレス、部門、案件、ファイルはすべてダミーデータです。実際の会社情報、SharePoint／OneDriveリンク、共有フォルダパスは登録しないでください。

現在はSTEP 7まで実装済みです。SharePoint／OneDrive文書はコピーしたリンクを登録し、共有フォルダ文書は会社PCの読み取り専用APIを介して開きます。文書参照用のTenant ID、Client ID、管理者同意は不要です。Microsoft Graphの社内名簿検索は任意機能で、未設定時はローカルの承認者マスタだけで動作します。
...
- 共有フォルダは完全なファイルパスを登録し、会社PCの許可ルート内だけを読み取り専用で開く
- メール作成・編集・送信画面はUIデモ
...
一般的なブラウザからSMB共有へ直接アクセスしません。STEP 7では、会社PCのバックエンドが許可フォルダ内のファイルだけを読み取り専用で返す最小機能を実装しました。

1. `.env`の`SHARED_FOLDER_ROOTS`へ、参照を許可する絶対フォルダを`;`区切りで設定
2. 新しい回覧で「共有フォルダ」を選択
3. `\\fileserver\共有\製品仕様書.pdf`のような完全なファイルパスを追加
4. 案件画面の「文書を開く」から会社PCのAPIを介して取得

PDFはブラウザ表示、Excel・Word・PowerPointはダウンロード扱いです。許可ルート外、相対パス、未対応形式、存在しないファイルは拒否します。ファイル一覧、書き換え、削除は行いません。STEP 12では一覧選択、監査ログ、サービスアカウント運用などを追加検討します。
...
- 共有フォルダの完全パス登録、ファイル名自動入力、重複防止
- 会社PCの許可ルート内に限定した共有フォルダ文書の読み取り専用起動
- 社内名簿ユーザーを回付ルートへ直接追加
- 社内名簿ユーザーを承認者マスタへ登録
...
- 共有フォルダは`SHARED_FOLDER_ROOTS`の許可ルート内に限定し、パス逸脱とシンボリックリンクによる逸脱を拒否
- 共有フォルダAPIはPDF、Excel、Word、PowerPointの読み取りだけを許可
...
- 共有フォルダ起動には会社PCバックエンドの起動、`SHARED_FOLDER_ROOTS`設定、会社PC実行ユーザーの読み取り権限が必要です
- GitHub Pages版には共有フォルダAPIがないため、実ファイルは開けません
- PDFへの捺印有無を自動判定しません
...
- [x] STEP 7：会社環境向けOneDrive起動経路／共有フォルダ読み取り専用起動
- [ ] STEP 8：会社PCバックエンドへ回覧情報を保存
- [ ] STEP 9：実回覧
- [ ] STEP 10：NG・差し戻し
- [ ] STEP 11：途中ルート編集
- [ ] STEP 12：共有フォルダの一覧選択・運用強化
